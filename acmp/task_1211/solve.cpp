...
BigInteger treesDown(
	...
	BigInteger A_SKIP = BigInteger::valueOf(aSkip);
	BigInteger B_SKIP = BigInteger::valueOf(bSkip);
	BigInteger N = BigInteger::valueOf(n);
	return A * (N - N / A_SKIP) + B * (N - N / B_SKIP);
}
...
long long binSearch(
	...
	long long left = 1;
	long long right = n;
	const BigInteger NEED = BigInteger::valueOf(need);
	while (left < right) {
...
int main() {
	...
	const long long n = 2*(long long)1e18;
