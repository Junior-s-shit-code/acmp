...
int main() {
    ...
    std::vector<std::vector<std::vector<int>>> prevCount(2, std::vector<std::vector<int>>(10, std::vector<int>(10, 0)));
    std::vector<std::vector<std::vector<int>>> curCount(2, std::vector<std::vector<int>>(10, std::vector<int>(10, 0)));
    ...
    for (int digitA = 0; digitA < 10; digitA++) {
        for (int digitB = 0; digitB < 10; digitB++) {
            ...
            curCount[extra][digitA][digitB] = 1;
        }
    }

    for (int len = 1; len < n; len++) {
        curCount.swap(prevCount);
        for (int curA = 0; curA < 10; curA++) {
            for (int curB = 0; curB < 10; curB++) {
                curCount[0][curA][curB] = 0;
                curCount[1][curA][curB] = 0;
                    ...
                    for (int prevA = 0; prevA < 10; prevA++) {
                        for (int prevB = 0; prevB < 10; prevB++) {
                            if (curA != prevA && curB != prevB) {
                                curCount[extra][curA][curB] += prevCount[prevExtra][prevA][prevB];
                                curCount[extra][curA][curB] %= MOD;
                            }
                        }
                    }
                }
