...
struct Sensor {
	...
	bool isIntersecWith(const Sensor &second) const{
		int di = i - second.i;
		int dj = j - second.j;
...
void dfs(const int v,
		 ...
		 std::vector<bool> &was) {
	was[v] = true;
	if (sensors[v].isIntersecWith(Sensor{ 0, sensors[v].j, 0 })) {
		top = true;
	} 
	if (sensors[v].isIntersecWith(Sensor{ sizeI, sensors[v].j, 0 })) {
		bot = true;
	}
	if (sensors[v].isIntersecWith(Sensor{ sensors[v].i, 0, 0 })) {
		left = true;
	}
	if (sensors[v].isIntersecWith(Sensor{ sensors[v].i, sizeJ, 0 })) {
		right = true;
	}
	for (int i = 0; i < (int)was.size(); i++) {
		if (!was[i] && sensors[v].isIntersecWith(sensors[i])) {
